...
<table class="TableBlock" width=100% height=100%>
  <tr>
    <td width="100" valign="top">
  	...
  	<td valign="top" style="position:absolute;width:90%;height:97%;">
<?
$AIP_WIDTH ="100%";
$AIP_HEIGHT = "100%";
...
function aip_upload()
{
	...
	*/
    try{	
        ...
        URL = "<?=((isset($_SERVER['HTTPS'])&&$_SERVER['HTTPS']=='on')?'https':'http').'://'.$_SERVER['HTTP_HOST']?>/module/AIP/upload.php";
...
function AIP_SetFields()
{
    ...
    for(var i=0;i<el.length;i++)
    {
        ...
        if(oName && oName.indexOf("DATA_")==0)
        {
	        ...
	        oAIP.SetValue(el[i].title,"");
            oAIP.SetValue(el[i].title,el[i].value);
        }
    }
    //处理附件区域
    if($("macro_attach"))
