				...
				if(jQuery('#intrustData').find('.result_error').length > 0){//有错误提示则不提交
					...
					jQuery("input[type='text'][name^='intrust_run_']").each(function(){
						...
						jQuery.ajax({
							...
							cache: false,
							async: false,
							data: "action=submit&run_id="+run_id+"&prcs_key_id="+prcs_key_id+"&prcs_id="+prcs_id+"&flow_prcs="+flow_prcs+"&to_user="+to_user+"&mobile_check="+mobile_check+"&msg_check="+msg_check+"&sms_content="+sms_content+"&run_name="+run_name+"&flow_id="+flow_id,
							error: function(msg){
								alert(msg);
