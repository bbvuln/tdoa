...
define('tDesktop19/tDesktop.IM',function(require, exports, module){
    var $ = window.jQuery;
    var Comet = {
        init: function(c){
            var self = this;
            self.tDesktop19 = c.tDesktop19;
            //newsms online
        ...
        noc_mon: function(){
            var self = this;
            //trigger noc load
            self.tDesktop19.EventManager.trigger('loadNoc:load');
        },
        //settimeout 3s comet
        sms_mon: function(){
            var self = this;
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.indexOf("1") >= 0)
                    {
                        ...
                        {
                            //trigger load sms
                            self.tDesktop19.EventManager.trigger('LoadSms:load');
                        }
                    }
                }
            });
    ...
    var Chat = {
        ...
        bindEvent: function(){
            var self = this;
            self.tDesktop19.EventManager.on('LoadSms:load',function() {
                self.loadSms();
            });
            //read msg
            self.tDesktop19.EventManager.on('msg:read', function(sms){
                var online = null;
                $.ajax({
                    type: 'GET',
        ...
        showChat: function(){
			...
			if(window.IS_SIGN==0){
				$(".chat-wrapper.active").css("bottom","65px");
			}
        },
        hideChat: function(){
            ...
            $('#chat-wrapper').hide();
			if(window.IS_SIGN==0){
				$(".chat-wrapper.active.mini").css("bottom","-259px");
			}
        },
                ...
                success: function(data){
                    ...
                    for(var i=0; i< array.length; i++)
                    {
                        ...
                        if(!bFound){
                            ...
                            self.tDesktop19.EventManager.trigger('message:create', array[i]); 
                        }
                    }
                    self.formatSms(); 
                    //send and receive sms，trigger recentlist update
                    self.tDesktop19.EventManager.trigger('msg:update');
        ...
        sendMsg: function(){
            ...
            $.ajax({
                ...
                data: {'TO_UID':user_id,ACTION_TYPE:"sms", 'TD_HTML_EDITOR_CONTENT':content, 'charset':'utf-8'},
                dataType: 'text',
                success: function(d){
                    var d = JSON.parse(d);
                    var date = new Date();
...
//                    alert(e);
        ...
        bindEvent: function(){
            ...
            self.tDesktop19.EventManager.on('msg:update', function(sms){
                self.render();
            });
            //in multisend
            $("#tosend").click(function(){
                self.openSend();   
                self.tDesktop19.EventManager.trigger('multisend:in');
                self.tDesktop19.EventManager.trigger('MultiSend:initUpload');
            });
            //recent click item
            $(".recentlist").delegate(".recentItem", "click", function(){
                ...
                self.tDesktop19.EventManager.trigger('msg:read', sms);
            });
            //update recentlist
    ...
    var MultiSend = {
        ...
        bindEvent: function(){
            var self = this;
            self.tDesktop19.EventManager.on('MultiSend:initUpload',function(){
                self.initUpload();
            });   
        ...
        send: function(userid, username, content, attachidstr,attachnamestr, attachstr){
            ...
            $.ajax({
                ...
                data: { 
                    ...
                    username: username,
                    ACTION_TYPE:"sms",
					TD_HTML_EDITOR_CONTENT: content,
        ...
        showGetMore: function(){
            ...
            if(scrollheight > height){
                ...
                var topValue = $("#user-tags").position().top + height+30;
                $("#getmore").show();
                //more bnt position
        ...
        initUpload: function(){  
            ...
            self.tDesktop19.EventManager.on('multisend:in',function(){
        ...
        addFile: function(file){
            ...
            $list.append( '<div id="' + file.id + '" class="item" title="' + file.name +'">' +
                '<img src="" /><span class="filename">'+file.name+'</span><input type="hidden" class="fileid" /><div class="close"></div>' +
            '</div>' );
            //console.log(this,arguments);
            var $li = $( '#'+file.id );
    ...
    var Org = {
        ...
        bindEvent: function(){
            ...
            //comet online
            self.tDesktop19.EventManager.on('online:comet',function(){
                $('#user_online').triggerHandler('_show');
                self.getOnlineCount();
            });    
            //stop Comet
            self.tDesktop19.EventManager.on('online:stopcomet',function(){
    ...
    exports.IM = {
        init: function(c){
            this.tDesktop19 = c.tDesktop19;
            Comet.init({tDesktop19: this.tDesktop19});
            MultiSend.init({tDesktop19: this.tDesktop19});
            Chat.init({tDesktop19: this.tDesktop19});
            RecentList.init({tDesktop19: this.tDesktop19});
            Org.init({tDesktop19: this.tDesktop19});
        },
        Comet: Comet,
        Chat: Chat,
