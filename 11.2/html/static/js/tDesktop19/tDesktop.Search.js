define('tDesktop19/tDesktop.Search',function(require,exports,module) {
    var $ = window.jQuery;
    require('backbone');
    ...
    searchModule.initBind = function() {
		...
		//清空关键字
        $(".search-container").delegate("i.close-btn", "click", function(){
            $("#search-input").val("");
			$("i.close-btn").hide()
        });

		//当输入框获取焦点时
		$(".search-container #search-input").keyup(function(event) {
			//如果输入框的值不为空
			if($("#search-input").val() != ""){
				$("i.close-btn").show();
			}else{
		...
		//当输入框失去焦点时
		$(".search-container #search-input").blur(function(event) {
			//如果输入框的值不为空
			if($("#search-input").val() != ""){
				$("i.close-btn").show();
			}else{
		...
		if(tDesktop19.isTouchDevice()){
			...
			tDesktop19.EventManager.trigger('iscroller:create','search-results-container');
		}
        ...
        $('.search-results-tabs li').click(function(){
			...
			tDesktop19.EventManager.trigger('iscroller:refresh','search-results-container');
        });
    ...
    function search(keyword,searchType,curPage,pageLimit) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                self.$el.animate({height:'80%'},300);//展开搜索框
