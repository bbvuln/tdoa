...
function gopager(){
	if(parseInt(jQuery('#sp_1_pager').html()) < parseInt(jQuery('#gopage').val())){
		alert(sprintf(td_lang.general.workflow.msg_89, parseInt(jQuery('#sp_1_pager').html())));
		return false;
	}
	jQuery('.ui-pg-input').val(jQuery('#gopage').val());
	jQuery("#gridTable").jqGrid('setGridParam',{url:"data/archive_getdata.php?pageType="+pageType,page:jQuery('#gopage').val()}).trigger('reloadGrid');	
}
...
function set_user()
{
    ...
    if(document.form1.FLOW_QUERY_TYPE.value==6){
    	SET_USER.style.display="inline";
    }else{
    	SET_USER.style.display="none";
	}
	if(query_export_area == 6)
	{
