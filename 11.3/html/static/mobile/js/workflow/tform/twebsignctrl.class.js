						  ...
						  '<% } else{ %>'+
                             ...
                             '<% if(writable){ %>'+
								'<%if(sealType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
									'<span class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_END" onclick="goToWebSeal(\'<%=id%>\',\'<%=datafld%>\')">'+
										'<span>重盖</span>'+
									'</span>'+
								'<%}if(writeType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
									'<span class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_WRITE_END" onclick="getWebWritePage(\'<%=id%>\',\'<%=datafld%>\',\'<%=lineColor%>\')">'+
										'<span>重写</span>'+
									'</span>'+
								'<%}%>'+
                            '<% } %>'+
                             '<span style="display:none;" class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_START" onclick="goToWebSeal(\'<%=id%>\',\'<%=datafld%>\')">'+
                                '<span>盖章</span>'+
                             '</span>'+
							 '<input id="web_sign_<%=id%>" name="web_sign_<%=id%>" type="hidden" value=1>'+
						  	 '<% } else{ %>'+
								'<% if(writable){ %>'+
								 '<%if(sealType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
    						  	 '<span class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_START" onclick="goToWebSeal(\'<%=id%>\',\'<%=datafld%>\')">'+
                                    '<span>盖章</span>'+
                                 '</span>'+
								 '<%}if(writeType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
								 '<span class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_START" onclick="getWebWritePage(\'<%=id%>\',\'<%=datafld%>\',\'<%=lineColor%>\')">'+
                                    '<span>手写</span>'+
                                 '</span>'+
								 '<%}%>'+
                                 ...
                                 '</span>'+*/
								 '<%if(sealType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
                                 '<span style="display:none;" class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_END" onclick="goToWebSeal(\'<%=id%>\',\'<%=datafld%>\')">'+
                                    '<span>重盖</span>'+
                                 '</span>'+
								 '<%}if(writeType == 1 && mobile_auth_websign_code != "" && mobile_auth_websign != false){%>'+
								'<span style="display:none;" class="ui-btn ui-btn-primary ui-btn-s" data_field="<%=datafld%>" name="<%=id%>_WRITE_END" onclick="getWebWritePage(\'<%=id%>\',\'<%=datafld%>\',\'<%=lineColor%>\')">'+
                                    '<span>重写</span>'+
                                 '</span>'+
								 '<% }} %>'+
