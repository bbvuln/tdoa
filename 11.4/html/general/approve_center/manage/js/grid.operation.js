jQuery(document).ready(function(){
    ...
    jQuery(document).on('click', '.users-add', function(){
        ...
        if(prcs_next){
			...
			if(prcs_next_id.indexOf('_') > 0)
			{
				var prcs_next_id_new = prcs_next_id.split('_');
				var prcs_next_id = prcs_next_id_new[1];
			}
    ...
    //批量转交操作
    jQuery('#sole_form_submit').on('click', function(){
        // 用户自定义js脚本执行程序
        var beforeCustomScript = jQuery("#turnBeforeCustomScript").val();
        if(typeof beforeCustomScript !== 'undefined')
        {
        ...
        else
        {
            ...
            if((jQuery("li[id^='next_prcs_'][class*='active'][next_prcs_name!='结束流程']").length != jQuery("[id^='work-next-prcs-block-']").find('li').length)){
				alert(td_lang.general.workflow.msg_244);
				return false;
            }
            ...
            try{
                ...
                jQuery("li[id^='next_prcs_'][class*='active']").each(function(i, v){
                    var obj_choose_prcs = jQuery(v);
                    var flow_prcs = obj_choose_prcs.attr("id").split('next_prcs_')[1];
                    if(jQuery('#PRCS_USER'+flow_prcs).val() == ''){
                        throw td_lang.general.workflow.msg_245;
                        return false;
                    }
                    ...
                    if(obj_choose_prcs.attr("id").indexOf('_0') > 0 && (obj_choose_prcs.attr('prcs_back') == 0 || typeof(obj_choose_prcs.attr('prcs_back')) == 'undefined')){
                        chose_obj.val("");
                    }else{
                        // chose_obj.val(flow_prcs);
						chosePrcs += flow_prcs+',';
                    }
                    // jQuery(window.frames["work_form_data"].document).find('#next_prcs_num').val(obj_choose_prcs.attr("id").substr(10));
                });
				if(chosePrcs != '')
				{
					chosePrcs = chosePrcs.substring(0,chosePrcs.length - 1);
				}
				chose_obj.val(chosePrcs);
				jQuery(window.frames["work_form_data"].document).find('#RUN_ID_STR').val(run_id);
				jQuery(window.frames["work_form_data"].document).find('#ID_STR').val(prcs_key_id);
            }catch(e){
                alert(e);
                return false;
            }
            jQuery(this).attr("disabled", true);
            var s_prcs_to = "";
            jQuery('ul[id^="workPrcsData_"]').find("li[id^='next_prcs_']").each(function(){
                var a_prcs_id = jQuery(this).attr('id').split('next_prcs_');
                if(a_prcs_id.length == 2){
                    var s_prcs_id = a_prcs_id[1];
                    // if(s_prcs_id == "END"){
                        // s_prcs_to += "0,";
                    // }else{
                        s_prcs_to += s_prcs_id+",";
            ...
            jQuery("#gridTable").jqGrid({
                ...
                loadComplete:function(xhr){
                    ...
                    else
                    {
                        ...
                        sizeArr.vHeight = sizeArr.vHeight - 40;
                        jQuery("#gridTable").jqGrid('setGridHeight',sizeArr.vHeight);
