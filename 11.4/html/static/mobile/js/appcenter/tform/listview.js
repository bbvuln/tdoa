                ...
                if (field.writable == false) {
                    ...
                    if (field.options != undefined) {
                        if ($('#scroller_list_detail [name="' + field_id + '"]').length > 0 && $('#scroller_list_detail [name="' + field_id + '"]').prop("nodeName").toUpperCase() == "SELECT") {//.ui-page-active
                            ...
                            $('#scroller_list_detail [name="' + field_id + '"]').html(s_option);//.ui-page-active
...
define('RListField', function (require, exports, module) {
    ...
    function emptyHelper(c, v) {
        if (v && v.fieldtype == "RCalcCtrlField") {
            ...
            var opts = '';
            $.each(c, function (i, value) {
                opts = opts.length === 0 ? '<a class="ui-attch-img-wrap pda_attach" is_image="1" href="javascript:void(0);" _href="' + value.attach_url.down + '"><span>' + value.attach_name + '</span></a>' : opts + '<a class="ui-attch-img-wrap pda_attach" is_image="1" href="javascript:;" _href="' + value.attach_url.down + '"><span>' + value.attach_name + '</span></a>';
            })
            return c = opts
            ...
            var unval = ''
            var opts = ''
            unval =  v.value.replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, '$&,');//千分位
            if(v.kind == "upCase"){
    ...
    var RAttachmentField = Base.extend({
        ...
        bindEvent: function () {
            ...
            var multi = self._config.multi;
              var platForm = tMobileSDK.platForm;
            self.$el.find('button').on('click', function () {
              if(platForm =='qywx' || platForm =='dd' || platForm == 'wx'){
                alert('客户端暂不支持上传附件');
                return false
              }
