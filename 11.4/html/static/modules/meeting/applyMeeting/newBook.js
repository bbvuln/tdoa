 ...
 */
window.isMeetingPass = true;
window.isChoice = false;
function WeekBooker(cfg) {
  ...
  function doDragHelper() {
    ...
    el.mousedown(function(e) {
      ...
      helper.css({
        left: e.clientX + ___getPageScroll()[0] + "px",
        top: e.clientY + "px"
      });
      ...
      if (helper.clientX > 0 && helper.clientY > 0) {
        ...
        // });
        // helper.css({
        //   left: helper.clientX + ___getPageScroll()[0] + "px",
        //   top: helper.clientY + "px"
        // });
...
function Booker(cfg) {
  ...
  $.each(meetingInfo,function(index,value){
    $('.container').eq(index).attr('mr-id',value.mr_id);
  });
  /**
   * 判断是否有冲突,时间段是否有交叉
   */
  ...
  function add(meeting) {
    ...
    c.width = c.limit==1441?2882:c.width;
