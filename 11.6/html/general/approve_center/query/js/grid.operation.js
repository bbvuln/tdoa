...
//    });
                ...
                jQuery("#gridTable").jqGrid({
                    ...
                    loadComplete:function(xhr){
                        ...
                        if(jQuery(this).getGridParam("records") == 0)
                        {
                            ...
                            sizeArr.vHeight = sizeArr.vHeight;
                            jQuery("#gridTable").jqGrid('setGridHeight',sizeArr.vHeight);
    ...
    jQuery(window).resize((function(t1, t2){
        ...
        return function(){
            t1 && clearTimeout(t1);
            t2 && clearTimeout(t2);
			//修正pc精灵打开工作查询，页面resize时列表宽度首次加载计算错误 tianlin
			if(window.external && typeof window.external.OA_SMS != 'undefined')
			{
				t1 = setTimeout(setTimeoutgetGridTableSize, 500);
				t2 = setTimeout(setTimeoutgetGridTableSize, 800);
			}else{
...
function set_user(obj)
{
	...
	var appendBeginName = jQuery(obj).attr('name') == 'FLOW_QUERY_TYPE' ? document.getElementById("BEGIN_DEPT_NAME") : document.getElementById("BEGIN_EXPORT_DEPT_NAME");
	...
	var select_val = jQuery(obj).find('option:selected').val();
	select_val = typeof(select_val) == 'undefined' ? document.form_export.exp_flow_query_type.value : select_val;
    if(select_val==10)
    {
		var appendName = jQuery(obj).attr('name') == 'FLOW_QUERY_TYPE' ? 'LEAVE_QUERY_TYPE' : 'LEAVE_EXPORT_TYPE';
        LEAVE_USER.style.display="inline";
		var url = "getUserOrDept.php?type=1"
		jQuery.get(url,function(data){
			if(data)
			{
				jQuery('#'+appendName).find('option').remove();
				var userArr = JSON.parse(data);
				var html = '<option value=""></option>';
				for(var i=0;i<userArr.length;i++)
				{
					html += '<option value="'+userArr[i]['USER_ID']+'">'+userArr[i]['USER_NAME']+'</option>';
				}
				jQuery('#'+appendName).append(html);
			}
		});
		appendBeginName.style.display="none"
    }else
    {
        LEAVE_USER.style.display="none";
		if(select_val==11)
		{
			appendBeginName.style.display="inline";
			var url = "getUserOrDept.php?type=2"
			jQuery.get(url,function(data){
				if(data)
				{
					jQuery('#'+appendBeginName.id).find('option').remove();
					var deptArr = JSON.parse(data);
					var html = '<option value="all">全部</option>';
					for(var key in deptArr)
					{
						var DeptId = key.replace(/key_/,'');
						html += '<option value="'+DeptId+'">'+deptArr[key]+'</option>';
					}
					jQuery('#'+appendBeginName.id).append(html);
				}
			});
		}else
		{
