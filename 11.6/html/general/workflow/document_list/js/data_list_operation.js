...
function handle_work(menu_flag, run_id, prcs_key_id, flow_id, prcs_id, flow_prcs){ //办理工作
	var url = "/general/workflow/document_list/input_form/?actionType=handle";
...
function flow_view(run_id, flow_id){
	...
	var url = "/general/workflow/document_list/flow_view/?";
	url += "RUN_ID="+run_id;
	url += "&FLOW_ID="+flow_id;
...
function view_work(menu_flag, run_id, prcs_key_id, flow_id, prcs_id, flow_prcs){ //查看工作
	...
	var url = "/general/workflow/document_list/print/?";
	url += "RUN_ID="+run_id;
	url += "&MENU_FLAG="+menu_flag;
	url += "&PRCS_KEY_ID="+prcs_key_id;
	url += "&FLOW_ID="+flow_id;
...
function call_back(RUN_ID,PRCS_ID,FLOW_PRCS)
{
    ...
    if(window.confirm(msg))
    {
        var url="/general/workflow/document_list/call_back.php?RUN_ID="+RUN_ID+"&PRCS_ID="+PRCS_ID+"&FLOW_PRCS="+FLOW_PRCS;
        jQuery.get(url,{},function(data){
            if(data==1)
