...
//转交
function transmission(RUN_ID,PRCS_KEY_ID,FLOW_ID,PRCS_ID,FLOW_PRCS,RUN_NAME,PRCS_NAME,FLOW_TYPE)
{
    ...
    /*jQuery.ajax({
        type: "POST",
        url: "/general/approve_center/list/input_form/check_feedback_signed.php",
        async: false,
        data: {
            "run_id": RUN_ID,
            "flow_id": FLOW_ID,
...
function addBatchPrcsJsonData(data)
{
	for(var key in data)
	{
		var runId = key.substr(4);
		var Data = {"prcsData":data[key]['prcsData']};
		var template = jQuery.templates("#workPrcsTmpl");
		var htmlOutput = template.render(Data);
		var runData = data[key]['prcsData'];
		jQuery("#workPrcsData_"+runId).html(htmlOutput);
		jQuery(data[key]['prcsData']).each(function(i, v){
			var prcs_name_obj = jQuery('#workPrcsData_'+runId+' li[id="next_prcs_'+v.prcs_num+'"]').find('a:first');
			...
			var prcs_obj_li = jQuery('#workPrcsData_'+runId+' li[id="next_prcs_'+v.prcs_num+'"]')
		...
		sync_deal = data[key].sync_deal; //并发
		turn_priv = data[key].turn_priv;// 强制转交
		// gather_node = runData.gather_node;// 强制合并
		if(sync_deal == 2)
		{
			jQuery('#prcs_title_'+runId).append('&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">'+td_lang.general.workflow.msg_242+'</font>');
		}
	...
	jQuery('.workflow-node').click(function(){
		var sync_deal = $(this).parent().attr('sync_deal');
		var prcs_run_id = $(this).attr('prcs_run_id');
		...
		if(prcs_info_arr.length != 4){
			return false;
		}
		var prcs_num = prcs_info_arr[2]+'_'+prcs_info_arr[3];
		var new_prcs_num = prcs_info_arr[3];
		...
		if(sync_deal == 0)
		{
			...
			{
				//$this.siblings().removeClass('active');
				if(flow_prcs == new_prcs_num)
				{
					jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='next_prcs_"+prcs_num+"']").addClass('active');
					jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='next_prcs_"+next_prcs_id+"']").removeClass('active');
					return false;
				   
				}else
				{
					jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='next_prcs_"+prcs_num+"']").addClass('active');
					jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='next_prcs_"+flow_prcs+"']").removeClass('active');
				}
			}
			if(prcs_info_arr[3] === "0" && $this.attr("prcs_back") == undefined)
			{
				jQuery("#work-next-prcs-block-"+prcs_run_id).html('');
			...
			if(prcs_type == 3) //柔性转交
			{
				if(flow_prcs == new_prcs_num) //柔性步骤
				{
			...
			jQuery("#work-next-prcs-block-"+prcs_run_id).html(htmlOutput);
			...
			if( $this.attr('user_lock') !== '0'){
                ...
                if($this.attr('auto_type')!==""&&jQuery(".user-tags").length!==0)
                {
                    ...
                    jQuery('#work-next-prcs-block-'+prcs_run_id).find('.close').hide();//yzx 隐藏人员删除
                }
		...
		}else if(sync_deal == 1)//允许并发
		{
			if($this.hasClass('active')){
                ...
                jQuery("#work-next-prcs-block-"+prcs_run_id+" [prcs_id_next='"+prcs_num+"']").remove();
                jQuery('#all_check').removeClass('active');
            }else{
                $this.addClass('active');
                jQuery('#all_check').trigger('refreshCheckStatus');
                if(prcs_info_arr[3] === "0"  && typeof $this.attr("prcs_back") == 'undefined'){
                    jQuery("#work-next-prcs-block-"+prcs_run_id).html('');
                    jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id!='next_prcs_"+prcs_run_id+"_0']").removeClass('active');
                    jQuery('#all_check').removeClass('active');
                    // jQuery('div[data_type="next"]').hide();
                    return false;
                } else if (prcs_info_arr[3] === "0" && typeof $this.attr("prcs_back") != 'undefined') {  // 当前点击的是结束节点，并且没有返回步骤
                    jQuery("#work-next-prcs-block-"+prcs_run_id+" [prcs_id_next!='"+prcs_run_id+"_0']").remove();  // 将人员选择区域清空
                    jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id!='next_prcs_"+prcs_run_id+"_0']").removeClass('active');  // 取消所有不是结束节点的选中状态
                    jQuery('#all_check').removeClass('active');  // 取消全选选中状态
                } else {
                    jQuery("#work-next-prcs-block-"+prcs_run_id+" [prcs_id_next='"+prcs_run_id+"_0']").remove();  // 删除人员选择区的结束步骤
                    jQuery(".workflow-node[id='next_prcs_"+prcs_run_id+"_0']").removeClass('active');  // 取消结束节点的选中状态
                }
                // jQuery('#myModalForm').find('#SMS_CONTENT').val(sms_content);
                prcs_info_arr[3] === "0" && typeof $this.attr("prcs_back") == 'undefined' && jQuery(".workflow-node[id='next_prcs_"+prcs_run_id+"_0']").removeClass('active');
                // jQuery('div[data_type="next"]').show();
                var prcsBlockData = new Array();
                var prcs_uname = new Array();
                var prcs_uname_arr = $this.attr('prcs_uname').split(',');
                ...
                var template = jQuery.templates("#workNextPrcsUserTmpl");
                var htmlOutput = template.render(prcsBlockData);
                var current_index = jQuery("li[id^='next_prcs_'][prcs_run_id='"+prcs_run_id+"'][class*='active']").index($this);
                var before_insert_id = '';
                jQuery("li[id^='next_prcs_'][prcs_run_id='"+prcs_run_id+"'][class*='active']").each(function(i, v){
                    var unit_index = jQuery("li[id^='next_prcs_'][prcs_run_id='"+prcs_run_id+"'][class*='active']").index(jQuery(v));
                ...
                if(before_insert_id != ''){
                    ...
                    if(prcs_num_arr.length == 4){
                        var search_num = prcs_num_arr[2]+'_'+prcs_num_arr[3];
                        jQuery('li[prcs_id_next="'+search_num+'"]').before(jQuery(htmlOutput));
                    }
                }else{
                    jQuery("#work-next-prcs-block-"+prcs_run_id).append(htmlOutput);
                }
                jQuery('.workflow-node-title-text[title]').tooltip();
                if( $this.attr('user_lock') !== '0'){
                    jQuery("#TOP_FLAG_SHOW"+prcs_num).attachmenu();
		...
		}else if(sync_deal == 2)
		{
			if(prcs_info_arr[3] === "0"  && typeof $this.attr("prcs_back") == 'undefined')
			{
				...
				if($this.hasClass('active')){//取消
					...
					var nextPrcs0 = 'next_prcs_'+prcs_run_id+'_0';
					if(prcs_info_arr[3] === "0" && typeof $this.attr("prcs_back") != 'undefined'){
						jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='"+nextPrcs0+"']").addClass('active');  // 选中结束步骤节点
						jQuery("#work-next-prcs-block-"+prcs_run_id+" [prcs_id_next!='"+prcs_run_id+"_0']").remove();  // 将人员选择区域清空
						jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id!='"+nextPrcs0+"']").removeClass('active');  // 取消所有不是结束节点的选中状态
						var $li_obj = jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='"+nextPrcs0+"']");
					}else{
						jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id!='"+nextPrcs0+"'][condition!='false']").addClass('active');  // 选中条件不为false的步骤节点
						jQuery("#work-next-prcs-block-"+prcs_run_id+" [prcs_id_next='"+prcs_run_id+"_0']").remove();  // 将人员选择区域清空
						jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id='"+nextPrcs0+"']").removeClass('active');  // 取消选中的结束步骤节点
						var $li_obj = jQuery(".workflow-node[prcs_run_id='"+prcs_run_id+"'][id!='"+nextPrcs0+"'][condition!='false']");
					}
					// jQuery('div[data_type="next"]').show();
					$li_obj.each(function(i, v){
						$v = jQuery(v);
						 var prcsBlockData = new Array();
							...
							var prcs_num = prcs_num_arr[2]+'_'+prcs_num_arr[3];
							var prcsBlockData = [{
								'prcs_num':prcs_num, 
								'prcs_back': $this.attr('prcs_back'),
	...
	for(var key in data)
	{
		...
		{
			jQuery('[id^="next_prcs_"][prcs_run_id="'+runId+'"]').trigger('click');
		} else if (data[key].defaultNextPrcs) 
		{
			jQuery('#next_prcs_'+runId+'_'+data[key].defaultNextPrcs).click();
		}
	}
}
