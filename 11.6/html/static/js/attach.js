...
if (typeof (LoadScript) != "function") {
	function LoadScript(url) {
		...
		for (var i = 0; i < scripts.length; i++) {
			...
			if (src) {
				src = src.toLowerCase();
				if (src == url.toLowerCase() || src + ".gz" == url.toLowerCase() || src == url.toLowerCase() + ".gz")
					return;
			}
		}
...
LoadScript('/module/OC/ntkobackground.min.js?20191209');
...
function AddFile(file_type,divname)
{
   ...
   for(var i=0; i<el.length;i++)
   {
	...
	if(reg.test(attach_name)){
		alert("文件名"+attach_name+"包含[/\\'\":*?<>|]等非法字符");
		var strId = prefix+'_'+id;
		document.getElementById(strId).value = '';
		return false;
	}
	/* if(!(checkName(attach_name,"/")&&checkName(attach_name,"\\")&&checkName(attach_name,"..")&&checkName(attach_name,"!")&&checkName(attach_name,"@")&&checkName(attach_name,"#")&&checkName(attach_name,"$")&&checkName(attach_name,"%")&&checkName(attach_name,"^")&&checkName(attach_name,"&")&&checkName(attach_name,"*")&&checkName(attach_name,"{")&&checkName(attach_name,"}")&&checkName(attach_name,"[")&&checkName(attach_name,"]")&&checkName(attach_name,"〔")&&checkName(attach_name,"〕")&&checkName(attach_name,"〕") )){
		alert('上传文件名称出现非法字符');
		var strId = prefix+'_'+id;
		document.getElementById(strId).value = '';
		return false;
