	...
	_getInnerHtml: function() {
		...
		if(!nodeTitle){
			...
			if( opts.noLink || data.noLink ) {
				nodeTitle = "<span style='display: inline-block;padding=' class='" + opts.classNames.title + "'" + tooltip + ">" + data.title + "</span>";
//				this.tree.logDebug("nodeTitle: " + nodeTitle);
			}else{
				nodeTitle = "<a href='javascript:;' class='" + opts.classNames.title + "'" + tooltip + ">" + data.title + "</a>";
			}
		}
		res += nodeTitle;
		return res;
...
//		this.tree.logDebug("dtnode.onClick(" + event.type + "): dtnode:" + this + ", button:" + event.button + ", which: " + event.which);
		...
		if( targetType === "expander" ) {
			...
			var aTag = this.span.getElementsByTagName("a");
						//增加点击focus颜色  2019.3.13   whf  
						window.setTimeout(function () {
						   $('a.dynatree-title').css('color','#444659')
						aTag[0].style.color='#0e86fe'
					   },80)
			if(aTag[0]){
				// issue 154
