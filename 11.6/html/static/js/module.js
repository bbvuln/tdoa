...
function navMenuClick(e) {
    ...
    if (jQuery(self).attr("block") == "online") {
        userSaveAndBind.initOnline(id);
    }
};
...
var userSaveAndBind = {
    ...
    //修正部门tab下全选后，切换至在线，在线未选中的情况
    initOnline: function (id) {
        var selectedIds = userSaveAndBind[id];
        jQuery('#online_item .block-right-item').removeClass('active');
        jQuery('#online_item .block-right-item').each(function(){
            var $this = jQuery(this);
            var user_id = $this.attr('user_id');
            if(selectedIds.indexOf(user_id) >= 0){
