...
whenReady(function(){
	// if(top.document.getElementById('wm_div_id') != undefined){
		// return;
	// }
	var WmManager = {
		configs: {
		...
		fetchConfigs: function(){
			var self = this			
			if(window.watermark_config){
				window.watermark_config.watermark_fontsize = parseInt(window.watermark_config.watermark_fontsize)+'px'
				self.render(window.watermark_config);
			}
			/*
			var url = ''
			if(true){
				...
				url = '/general/system/watermark/mock/getWaterMark.json'
			}					
			$.get(url,{"action":'getwatermark',"source": "pc"},function(response){
				if(typeof response == "string") response=JSON.parse(response)
				if(response.status){
					// self.configs = {
						// watermark_state: 1,
						// watermark_txt: '一段水印文字',
						// watermark_x:20,
						// watermark_y:20,
						// watermark_rows:0,
						// watermark_cols:0,
					...
					self.configs.watermark_fontsize = parseInt(self.configs.watermark_fontsize)+'px'
					self.render(self.configs);
				}
			})
			*/
		...
		restore: function(){
			if(this.configs.watermark_state){
				this.render(this.configs);
			}
		},
		render: function(configs){
			try{
				if(configs){
					var _configs = {};
					_configs.watermark_txt = configs.watermark_txt;
					_configs.watermark_height = 80;
					_configs.watermark_width = 100;
					_configs.watermark_x_space = 0;
					_configs.watermark_y_space = 0;  
					_configs.watermark_color = configs.watermark_color;
					_configs.watermark_fontsize = configs.watermark_fontsize;
					_configs.watermark_alpha = configs.watermark_alpha;
					_configs.watermark_angle = configs.watermark_angle;
					watermark.load(_configs)
				}
			}catch(e){
				console.log(e)
			}
