        ...
        getChartSetting:function(config, chartRow, chartCol,listconfig){
            ...
            if(config.categoryRegion){
                for(var guid in fieldsMap){
                    ...
                    if(field.type === 'list'){
                        if(field.subFields){
                            var subField = field.subFields.filter(function(subField){subField.guid === config.categoryRegion[0]});
            ...
            value = me.ResetData(list_item.chartval);
                var subFieldNameMap = [];
                list_item.subFields.map(function(subField){subFieldNameMap[subField.guid] = subField.title});

                if(config.orientation == '2'){
                    value.map(function(row, row_idx){
                        categories.push(row.data[config.categoryRegion[0]]['value']);

                        config.dataRegion.map(function(guid, idx){
            ...
            if(series_count > 0 && config.chartOptions && config.chartOptions.trendMethod){
                ...
                series[0]['data'].map(function(val, idx){
                    var amount = 0;
