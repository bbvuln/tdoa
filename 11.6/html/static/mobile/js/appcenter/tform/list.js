define('ListCtrl', function (require, exports, module) {
    ...
    var ListCtrl = Base.extend({
        initialize: function (config) {
            ...
            this._render();
            this.pageSize = config['defaultPageSize'] && config['pageSize'] ? parseInt(config['pageSize']) : 10;
            //this.all_page = {};
        ...
        renderherData: function () {
            ...
            if (self._config.hasGather) {
                self._config.subFields.map(function(item, index){
        ...
        //             ;
        //             subfield = subfield[0];
        //             var old_value = parseFloat(old_value[0].value || 0);
        //             subFieldValue = parseFloat(subFieldValue[0].value || 0);
        //             console.log(gather_val, old_value, subFieldValue)
        ...
        //                     break;
        //             }
        //             gatherData[subFieldId] = self.getFloat(gather_val);
        //             self._config.gatherData = gatherData;
        //         }
        //     }
        //     self.renderherData()
        // },
        getFloat: function (x) {//保留两位小数
            if (x != '.') {
        ...
        synchListValue: function (item) {//编辑列表详情后同步数据到列表中
            var value = this._config.value;
            var self = this;
            if (item.cfg.flag == 'new') {
                ...
                var all_old_row = [], all_new_row = [], arr_fixto = [], arr_gather_type = [];
                if(this._config['hasGather']){
                    //console.log(this._config)
                    for (var i in this._config.value) {
                        if (this._config.value[i].index == item.cfg.index) {
                            for(var j in this._config.value[i].data){
                                if(this._config['gatherData'] && this._config['gatherData'][this._config.value[i].data[j]['field_id']]){
                                    var field = this._config.value[i].data[j];
                                    all_old_row[field['field_id']] = parseFloat(field['value']);
                                    arr_gather_type[field['field_id']] = field['gatherType'];
                                    arr_fixto[field['field_id']] = parseFloat(field['fixTo']);
                                }
                            }
                            break;
                        }
                    }
                    //console.log(all_old_row)
                }
                for (var i in this._config.value) {
                    if (this._config.value[i].index == item.cfg.index) {
                ...
                if(this._config['hasGather']){
                    for (var i in this._config.value) {
                        if (this._config.value[i].index == item.cfg.index) {
                            for(var j in this._config.value[i].data){
                                if(this._config['gatherData'] && this._config['gatherData'][this._config.value[i].data[j]['field_id']]){
                                    all_new_row[this._config.value[i].data[j]['field_id']] = parseFloat(this._config.value[i].data[j]['value']);
                                }
                            }
                            break;
                        }
                    }
                    //console.log(all_new_row)
                    var totalCount = this._config['totalCount'];
                    for(var i in all_old_row){
                        var gather_val = parseFloat(this._config['gatherData'][i]);
                        switch(arr_gather_type[i]){
                            case 'sum':
                                gather_val = gather_val - all_old_row[i] + all_new_row[i];
                                break;
                            case 'avg':
                                gather_val = (gather_val * totalCount - all_old_row[i] + all_new_row[i]) / totalCount;
                                gather_val = gather_val.toFixed(arr_fixto[i]);
                                break;
                            case 'max':
                                if(gather_val < all_new_row[i]){
                                    gather_val = all_new_row[i];
                                }
                                break;
                            case 'min':
                                if(gather_val > all_new_row[i]){
                        ...
                        this._config['gatherData'][i] = gather_val;
                    }
                }
                //self.setherdata()
                self.renderherData()
        ...
        renderPage: function () { //渲染分页
            ...
            var html = '';
            store.all_page[self._config.field_id] = {page: this._config.page, pageSize: this._config.pageSize};
            if (this._config.value.length == 0) {
                ...
                for (var k in instance.value) {
                    ...
                    if (checkKindSubFields) {
                        for (var j in checkKindSubFields) {
                            ...
                            var subField = instance.value[k]['data'].filter(function (subField) {
                                                        return subField.field_id === instance2.field_id
                                                    });
                                instance2.value = subField[0].value
