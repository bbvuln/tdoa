...
function remindNextUser()
{
	var url = "/module/approve_center/engine/flow_reminders.php";
	var run_name = jQuery('span[class="run_name_content"]').text();
	var content = "提醒您尽快办理流水号："+q_run_id+",流程名："+run_name;
	var data = {"RUN_ID":q_run_id,"FLOW_ID":q_flow_id,"CONTENT":content};
	$.get(url,data,function(res){
		showMessage("成功发送催办信息");
	});
}
// 转交跳转至选步骤页面，同时增加保存
function turnWorkFlow(controlSource) {
    ...
    $.ajax({
        ...
        url: url,
        cache: false,
        data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'PRCS_ID': q_prcs_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_KEY_ID': q_prcs_key_id, 'FLOW_TYPE': flow_type,'feedback_content': feedback_content},
        beforeSend: function() {
            $.ProLoading.show();
        },
        success: function(data) {
...
function goOnWorkFlow(new_prcs_id_next, turn_action) {
    ...
    var VIEW_USER_NAME = $("input[name='VIEW_USER_NAME']").val() || ''; //传阅人
    var SHOW_VIEW = $("input[name='SHOW_VIEW']").val() || false; //是否显示传阅人
  ...
  //  var VIEW_USER_ID = $("input[name='VIEW_USER_ID']").val(); //传阅人
    ...
    $.each(prcs_id_next_arr, function(key, val) {
        if (val) {
            ...
            var timeLimit = $("#timeLimit").text() || '';//办理时限
            ...
            if(timeLimit.length > 0){
                POST_STR += "&TIME_OUT_" + val + "=" + timeLimit;
            }
            //新版经办人拼接
            var PRCS_USER_TMP = "";
            if (_cbems.length > 0) {
...
function saveSignWorkFlow() {
	...
	if((typeof CONTENT == "undefined" || (typeof CONTENT != "undefined" && CONTENT.match(/^\s*$/))) && hasAttach == false)
	{
		return;
	}
	$("#CONTENT").val();
...
function saveRelationWorkFlow() {
	var contactLength = jQuery('#relation_content').length;
	if(contactLength == 0)
	{
		return;
	}
    var params = {};
    params.flow_id = q_flow_id;
    params.run_id = q_run_id;
    params.flow_prcs = q_flow_prcs;
    params.prcs_id = q_prcs_id;
    params.prcs_key_id = q_prcs_key_id;
...
//进入数据源搜索
function goDataSelectionSearchPanel() {
    if(currMobileDataSelectedSelf && currMobileDataSelectedSelf.$config.query_field && currMobileDataSelectedSelf.$config.query_field) {
        if(!currMobileDataSelectedSearchFlag) {
            $('.ui-mask-panel').css('z-index',990);
            $('.ui-mask-panel').on('touchstart',function() {
                goOutToDataSelectionSearchPanel();
                // window.event?window.event.returnValue=false:e.preventDefault();
                // window.event?window.event.cancelBubble=true:e.stopPropagation();
            })

            $(".ui-data-select-search-panel-wrapper").animate({
                right:0,
            }, 300, 'ease-out');
            currMobileDataSelectedSearchFlag = true;
            currMobileDataSelectedSelf.setSearchData();
        }
    }else{
        alert('未设置查询条件！');
    }
...
//退出数据源搜索
function goOutDataSelectionSearchPanel() {
    $('.ui-mask-panel').css('z-index',-1);
    $(".ui-data-select-search-panel-wrapper").animate({
        right:'-85%',
    }, 300, 'ease-out');
    currMobileDataSelectedSearchFlag = false;
    history.back();
}
//切回数据源选择界面
function goOutToDataSelectionSearchPanel() {
    $('.ui-mask-panel').css('z-index',-1);
    $(".ui-data-select-search-panel-wrapper").animate({
        right:'-85%',
    }, 300, 'ease-out');
...
//增加
function goDataSelectionListPanel(params) {
    if(fieldManager && fieldManager.fieldsInstance) {
        var tmpObj =  fieldManager.fieldsInstance;
        $.each(fieldManager.fieldsInstance,function(index,item){
            if(item.$config.type === "TDataSelectionCtrl" && item.$config.data_type==="1"){
                for (var i in item.$config.keyItemStr){
                        if(params===i) {
                        currMobileDataSelectedSelf = item
                    }
                }
            }
        })
    }
    currMobileDataSelectedSelf.getMenuTypeOfMenuData();
}
    ...
    $.ajax({
        ...
        success: function(data) {
			...
			}else if(data == 'WORKBACK')
			{
                ...
                setTimeout("back2list('" + show_msg + "')", 1000);
                return;
			}else if(data == 'NOTPRCSUSER')
			{
				showMessage('不能将经办工作委托给主办人');
                return;
			}
        },
        error: function(data) {
