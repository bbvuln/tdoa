define('TExtDataSelectionCtrl', function(require, exports, module){
    ...
    var TExtDataSelectionCtrl = Base.extend({
		...
		bindEvent: function(){
            ...
            //备选项展开、收起
            $('#dataselection_container,#dataselectList').on('tap','.u-list-arrow-down',function() {
            ...
            //备选项展开、收起
            $('#dataselection_container,#dataselectList').on('tap','.u-list-arrow-up',function() {
                var val =  $(this).closest('.u-list-content-wrapper');
                val.css("background-color","#fff");
                val.css("height","68px");
                $(this).hide();
        ...
        getDatasByPage:function(page) {
            ...
            var datas = {
                ...
                "dataSrcName": self.$config.data_src_name,
                "data_control": self.$config.data_control,
                "datadbid": self.$config.data_db_id,
                ...
                "sidx":null,
                "sord": "asc",
                "objName":self.$config.objName,
                "itemStr":self.$config.itemStr,
            };
            var newObj = $.extend(true, {}, datas ,searchdata);
            $.ajax({
                type: 'GET',
                url: '/general/approve_center/list/input_form/getdatalink.php',
                cache: false,
                data: newObj,
                ...
                success: function(ret) {
                    ...
                    var dataArr = [];
                    var nameTmpArr = [];
                    titleArr = self.$config.data_field_name_filter.split('`');
                        ...
                        '<% for(var m=0;m<data.length;m++){ %>'+
                            '<div class="u-list-content-wrapper">'+
                                '<div class="u-list-contents-lists">'+
                                    '<div class="u-list-wrapper-left  ui-checkbox"><input type="radio" name="dataSelectedRadio" data-value="<%=name[m]%>" /></div>'+
