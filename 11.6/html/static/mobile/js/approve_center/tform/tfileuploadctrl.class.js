    ...
    var TFileUploadCtrl = Base.extend({
        ...
        parseHtml: function(d) {
            ...
            var tplHTML = '<% if(secret){ %>' +
                ...
                '<a href="javascript:void(0);" data-attach_id="<%=list[i].attach_id%>" data-attach_name="<%=list[i].attach_name%>" class="pda_attach pda_attach_file" edit_priv="<%=typeof(q_action_type) != "undefined" && q_action_type != "view" ? list[i].edit_priv : 0%>" _href="<%=list[i].attach_url%>"><span class="fileupload_attach_name"><%=list[i].attach_name%></span><span class="fileupload-item icon-delete">×</span></a>'+
        ...
        uploadFiles: function() {
			...
			tMobileSDK.selectFile({
				onSuccess: function(ret) {
					var d = ret;
					try{$.each(d, function(k, fileinfo){
						var attach_id = fileinfo.id;
						var attach_name = fileinfo.name;
						attach_name = attach_name.substring(0,attach_name.length-1);
						var attach_url = fileinfo.url;
						self.$list.append('<a href="javascript:void(0);" data-attach_id="'+fileinfo.id+'" data-attach_name="'+attach_name+'" class="pda_attach pda_attach_file" _href="'+fileinfo.url+'"><span class="fileupload_attach_name">'+attach_name+'</span><span class="fileupload-item icon-delete">×</span></a>');
