...
tMobileSDK.setRight = function(opts) {
    td.setRight({
        ...
        text: opts.text || '',	//添加头部右侧类型，默认是text，如果传值为icon，则客户端直接生成搜索框，并处理搜索框的点击事件，通过桥调用前端自定义方法返回搜索框输入的值。
		/*
		type: opts.type || 'text',	
		iconUrl: opts.iconUrl || '', //type为icon时的搜索结果地址
		onSearchChange: opts.onSearchChange || function(){},
		*/
        onSuccess: opts.onSuccess,
        onFail: opts.onFail
    })
...
tMobileSDK.videoConference = function(opts) {
	td.videoConference({
        data: opts.data,
        onSuccess: opts.onSuccess,
        onFail: opts.onFail
