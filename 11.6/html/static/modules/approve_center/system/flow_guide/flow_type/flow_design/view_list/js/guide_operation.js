...
jQuery(document).on("click", "#next_step_div_limit", function(){
...
jQuery(document).on("click", "#alternative_div_limit", function(){
	 var obj=jQuery('#alternative_div_limit').find('tr');
	 obj.removeClass('ui-selected');

	 
});

jQuery(document).on("click", "tr[class^='alternative_step']", function(){
    var parentId=jQuery(this).parent().attr('id');
	var selected_class = 'ui-selected';
    if(parentId=='alternative_next_limit'  )
	{
	...
	if(typeof(alternative_array_limit1)!="undefined" && (alternative_array_limit1 !=""))
	{
			
		alternative_array_next_limit();
	}
	if(typeof(alternative_array_limit2)!="undefined" && (alternative_array_limit2!=""))
	{
		
		alternative_array_go_limit();
	}	
...
//实时获取备选字段
function alternativelimit_array_from_next()
{
	    //清空
	    jQuery("#alternative_tr_limit").empty();
		var objNext= jQuery('#alternative_next').find('tr');
		var objNextLimit =  jQuery('#alternative_next_limit').find('tr');
		if(objNextLimit.length>0)
		{
			for(var i=0;i<=objNext.length-1;i++)
			{
				for(var j=0;j<=objNextLimit.length-1;j++)
				{
					if(objNextLimit[j].innerHTML == objNext[i].innerHTML )
					{
						 //objalternativelimit[j] = objNext[j];
						 objNext.splice(i,1);
					}
				}
		...
		if(objNext.length>0)
			{
				jQuery("#alternative_tr_limit").append(objNext.clone());
				
			}
	jQuery('#alternative_tr_limit').find('.alternative_step').removeClass('ui-selected');	
...
function TimeTurnWork(obj)
{
	if(obj ==1 || obj==2)
	{
		 jQuery("#turnWorkPrcs").css('display', 'block');
		
	}
	else
	{
        
		jQuery("#turnWorkPrcs").css('display', 'none');
	//	jQuery(".turnWork").css('display','none');
       }
}
//转入列表
...
function prcs_checkForm()
{
	...
	//必填字段会签控件数据处理
	var countersign = {};
	$('#countersign_item tr').each(function(i,v){
		var uid_ = $(v).find('td').eq(1).attr("uid");
		countersign[uid_] = {};
		$(v).find('input').each(function(){
			var type = $(this).attr('r_type');
			countersign[uid_][type] = $(this).is(":checked") == true ? 1 : 0;
		});
	});
	var r_sign = JSON.stringify(countersign);
	jQuery('#R_COUNTERSIGN_PRIV').val(r_sign);
	//可写字段数据处理
	...
	var prcs_to_limit ="";
	jQuery('#alternative_next_limit').find('tr').each(function(){
		var val = jQuery(this).attr('alternative_id');
		prcs_to_limit+=val + ",";
	});
	var prcs_to_limit_new = ""; 
	if(prcs_to!="")
	{
		var prcs_to_arr = prcs_to.split(',');
	
		if(prcs_to_limit!="")
		{
			var prcs_to_limit_arr = prcs_to_limit.split(',');
            for(var i=0;i<=prcs_to_limit_arr.length-1;i++)
			{
				for(var j=0;j<=prcs_to_arr.length-1;j++)
				{
					if( prcs_to_limit_arr[i] !== "" && prcs_to_arr[j] === prcs_to_limit_arr[i] )
					{
						prcs_to_limit_new += prcs_to_limit_arr[i] + ",";
					}
				}
			}
		}
	}
	jQuery("input[name='PRCS_TO_LIMIT']").val(prcs_to_limit_new);
	var item_id = "";
...
function alternative_array_go_limit()
{
	
	var jsonData = jQuery.parseJSON(alternative_array_limit2);
	if(!jsonData || jsonData == null)
	{
		return false;
	}
	var Data = {"runData":jsonData}
	var template = jQuery.templates("#alternative_trTmplLimit");
	var htmlOutput = template.render(Data);
	jQuery("#alternative_tr_limit").html(htmlOutput);
}
function alternative_array_next_limit()
{
	var jsonData = jQuery.parseJSON(alternative_array_limit1);
	var Data = {"runData":jsonData}	
	var template = jQuery.templates("#alternative_nextTmplLimit");
	var htmlOutput = template.render(Data);
	jQuery("#alternative_next_limit").html(htmlOutput);
}	




function selectAllLimit(type,is_jquery_ui)
{
    if(is_jquery_ui)
        var selected_class = 'ui-selected';
    else
    ...
    if(type=='left'){
        var objTrLength=jQuery('#next_step_tab_limit').find('tr').length;
        var objSelectLength=jQuery('#next_step_tab_limit').find('.'+selected_class).length;
        var obj=jQuery('#next_step_tab_limit').find('tr');
        if(objTrLength>0&objSelectLength<objTrLength){
            obj.addClass(selected_class);
            if(obj.hasClass(selected_class)){
                jQuery('#left_btn_limit').text(td_lang.system.workflow.msg_18);
            }else{
                jQuery('#left_btn_limit').text(td_lang.system.workflow.msg_17);
            }
        ...
        objTrLength=jQuery('#alternative_tab_limit').find('.alternative_step').length;
        objSelectLength=jQuery('#alternative_tab_limit').find('.'+selected_class).length;
        obj=jQuery('#alternative_tab_limit').find('.alternative_step');
        if(objTrLength>0&objSelectLength<objTrLength){
            obj.addClass(selected_class);
            if(obj.hasClass(selected_class)){
                jQuery('#right_btn_limit').text(td_lang.system.workflow.msg_18);
            }else{
                jQuery('#right_btn_limit').text(td_lang.system.workflow.msg_17);
            }
        }else if(objSelectLength>0){
            obj.removeClass(selected_class);
            if(obj.hasClass(selected_class)){
